...
<div class="post_view w-75">
  ...
  <div class="" style="margin-top:2px;">
    ...
    <p>{{ $post->event_at}}</p>
    <p><a href="{{ route('post.detail', ['id' => $post->id]) }}" class="post_title">{{ $post->title }}</a></p>
    <p>コメント数：{{ $post->postComments->count() }}</p>
    <p>閲覧数：{{$post->viewers->count()}}</p>
  ...
  @endforeach
  <div class="container">
    <a href="{{ route('category.input') }}">カテゴリーを追加</a>
    <br>
  ...
  @foreach($main_categories as $main_category)
  <p class="main_categories" category_id="{{ $main_category->id }}">{{$main_category->main_category}}</p>
  @foreach($sub_categories as $sub_category)
  @if($sub_category->post_main_category_id === $main_category->id)
  <div class="category_num{{ $sub_category->id }} sub_categories" style="display: none;">
